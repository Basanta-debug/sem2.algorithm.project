    ...
    def __init__(self, master):
        ...
        def Insert():
            if (len(self.admsn.get()) != 0):
                self.bill.insert(self.recpt.get(), self.name.get(), self.admsn.get(), self.date.get(),
        ...
        # def Search():
        #     self.list.delete(0, END)
        #     for row in self.bill.search(self.recpt.get(), self.name.get(), self.admsn.get(), self.date.get(),
        #                                   self.branch.get(), self.sem.get(), self.total.get(), self.paid.get(),
        #                                   self.due.get()):
        #         self.list.insert(END, row, str(' '))
        ...
        # ==================================================Frames===================================================
        self.Main_Frame = Frame(self.master, bg='Navajo white')
        self.Main_Frame.grid()

        self.Title_Frame = LabelFrame(
        self.Main_Frame, width=1350, height=100, bg='Navajo white', relief='ridge', bd=15)
        self.Title_Frame.pack(side=TOP)

        self.lblTitle = Label(self.Title_Frame, font=('arial', 40, 'bold'), text='FEE REPORT',
                              bg='navajowhite', padx=13)
        self.lblTitle.grid(padx=400)

        self.Data_Frame = Frame(self.Main_Frame, width=1350, height=350,
                           bg='Navajo white', relief='ridge', bd=15)
        self.Data_Frame.pack(side=TOP, padx=15)

        self.Frame_1 = LabelFrame(self.Data_Frame, width=850, height=350, bg='Navajo white', relief='ridge', bd=8,
                             text='Informations', font=('arial', 15, 'bold'))
        self.Frame_1.pack(side=LEFT, padx=10)

        self.Frame_2 = LabelFrame(self.Data_Frame, width=495, height=350, bg='Navajo white', relief='ridge', bd=8,
                             text='Fee Receipt', font=('arial', 15, 'bold'))
        self.Frame_2.pack(side=RIGHT, padx=10)

        self.List_Frame = Frame(self.Main_Frame, width=1350, height=150,
                           bg='Navajo white', relief='ridge', bd=15)
        self.List_Frame.pack(side=TOP, padx=15)

        self.Button_Frame = Frame(self.Main_Frame, width=1350, height=80,
                             bg='Navajo white', relief='ridge', bd=15)
        self.Button_Frame.pack(side=TOP)

        # ===================================================Labels================================================
        self.recpt_label = Label(self.Frame_1, text='Receipt No. : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.recpt_label.grid(row=0, column=0, padx=15, sticky=W)

        self.name_label = Label(self.Frame_1, text='Student Name : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.name_label.grid(row=1, column=0, padx=15, sticky=W)

        self.admsn_label = Label(self.Frame_1, text='Admission No. : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.admsn_label.grid(row=2, column=0, padx=15, sticky=W)

        self.Date_label = Label(self.Frame_1, text='Date : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.Date_label.grid(row=3, column=0, padx=15, sticky=W)

        self.branch_label = Label(self.Frame_1, text='Branch : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.branch_label.grid(row=4, column=0, padx=15, sticky=W)

        self.sem_label = Label(self.Frame_1, text='Semester : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.sem_label.grid(row=5, column=0, padx=15, sticky=W)

        self.total_label = Label(self.Frame_1, text='TOTAL AMOUNT : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.total_label.grid(row=2, column=2, padx=5, sticky=W)

        self.paid_label = Label(self.Frame_1, text='PAID AMOUNT : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.paid_label.grid(row=3, column=2, padx=5, sticky=W)

        self.due_label = Label(self.Frame_1, text='BALANCE : ', font=(
            'arial', 14, 'bold'), bg='Navajo white')
        self.due_label.grid(row=4, column=2, padx=5, sticky=W)

        # ==================================================Entries=================================================
        self.var_1 = DoubleVar(self.Frame_1, value='36265')
        self.d1 = datetime.date.today()
        self.date.set(self.d1)

        self.recpt_entry = Entry(self.Frame_1, font=(
            'arial', 14), textvariable=self.recpt)
        self.recpt_entry.grid(row=0, column=1, padx=15, pady=5)

        self.name_entry = Entry(self.Frame_1, font=(
            'arial', 14), textvariable=self.name)
        self.name_entry.grid(row=1, column=1, padx=15, pady=5)

        self.admsn_entry = Entry(self.Frame_1, font=(
            'arial', 14), textvariable=self.admsn)
        self.admsn_entry.grid(row=2, column=1, padx=15, pady=5)

        self.Date_entry = Entry(self.Frame_1, font=(
            'arial', 14), textvariable=self.date)
        self.Date_entry.grid(row=3, column=1, padx=15, pady=5)

        self.branch_entry = ttk.Combobox(self.Frame_1, values=(' ', 'CSE', 'IT', 'ETC/ET&T', 'Mechanical', 'Civil', 'EE', 'EEE'),
                                         font=('arial', 14), width=19, textvariable=self.branch)
        self.branch_entry.grid(row=4, column=1, padx=15, pady=5)

        self.sem_entry = ttk.Combobox(self.Frame_1, values=(' ', 'FIRST', 'SECOND', 'THIRD', 'FOURTH', 'FIFTH', 'SIXTH',
                                                       'SEVENTH', 'EIGHTH'), font=('arial', 14), width=19,
                                      textvariable=self.sem)
        self.sem_entry.grid(row=5, column=1, padx=15, pady=5)

        self.total_entry = Entry(self.Frame_1, font=(
            'arial', 14), width=10, textvariable=self.var_1, state='readonly')
        self.total_entry.grid(row=2, column=3, padx=8, pady=5)

        self.paid_entry = Entry(self.Frame_1, font=(
            'arial', 14), width=10, textvariable=self.paid)
        self.paid_entry.grid(row=3, column=3, pady=5)

        self.due_entry = Entry(self.Frame_1, font=(
            'arial', 14), width=10, textvariable=self.due)
        self.due_entry.grid(row=4, column=3, pady=7)

        # ==================================================Frame_2=================================================
        self.Display = Text(self.Frame_2, width=42, height=12,
                            font=('arial', 14, 'bold'))
        self.Display.grid(row=0, column=0, padx=3)

        # =============================================List box and scrollbar===========================================
        self.sb = Scrollbar(self.List_Frame)
        self.sb.grid(row=0, column=1, sticky='ns')

        self.list = Listbox(self.List_Frame, font=(
        ...
        # ==================================================Buttons=================================================
        self.btnSave = Button(self.Button_Frame, text='SAVE', font=(
            'arial', 14, 'bold'), width=10, command=Insert)
        self.btnSave.grid(row=0, column=0, padx=5, pady=5)

        self.btnDisplay = Button(self.Button_Frame, text='DISPLAY', font=(
            'arial', 14, 'bold'), width=10, command=View)
        self.btnDisplay.grid(row=0, column=1, padx=5, pady=5)

        self.btnReset = Button(self.Button_Frame, text='RESET', font=(
            'arial', 14, 'bold'), width=10, command=Reset)
        self.btnReset.grid(row=0, column=2, padx=5, pady=5)

        self.btnReset = Button(self.Button_Frame, text='UPDATE', font=(
            'arial', 14, 'bold'), width=10, command=Update)
        self.btnReset.grid(row=0, column=3, padx=5, pady=5)
        ...
        self.btnDelete = Button(self.Button_Frame, text='DELETE', font=(
            'arial', 14, 'bold'), width=10, command=Delete)
        self.btnDelete.grid(row=0, column=5, padx=5, pady=5)

        self.btnReceipt = Button(self.Button_Frame, text='RECEIPT', font=(
            'arial', 14, 'bold'), width=10, command=Receipt)
        self.btnReceipt.grid(row=0, column=6, padx=5, pady=5)

        self.btnExit = Button(self.Button_Frame, text='EXIT', font=(
            'arial', 14, 'bold'), width=10, command=Exit)
        self.btnExit.grid(row=0, column=7, padx=5, pady=5)
